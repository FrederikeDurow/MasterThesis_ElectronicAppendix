...
import os
import logging
# READ GOAL_AREA FROM BLACKBOARD
# READ GO_THROUGH_AREA FROM BLACKBOARD

# Please go to the kitchen gives: 
# [ChatCompletionMessageToolCall(id='call_DkfET25cLa9u36FHYgxnz9zk', function=Function(arguments='{"goal_area":"kitchen"}', name='go_to_goal_area'), type='function')]
...
class GoToArea(py_trees.behaviour.Behaviour):
    ...
    def update(self) -> Status:
        try: 
            ...
            self.logger.info("ACTIVATES THE NAVIGATION")
    ...
    def go_to_area(self,goal_area:str,go_through_area:str):
        ...
        try:
            if go_through_area in self.regions:
                ...
                # for i in range(len(go_through_area)):
                #     if go_through_area[i] in self.regions:
                #         pose = PoseStamped()
                #         pose.header.frame_id = 'map'
                #         pose.pose.position.x = self.regions[go_through_area[i]][0][0]
                #         pose.pose.position.y = self.regions[go_through_area[i]][0][1]
                #         pose.pose.orientation.x = 0.0
                #         pose.pose.orientation.y = 0.0
                #         pose.pose.orientation.w = self.regions[go_through_area[i]][1][0]
                #         pose.pose.orientation.z = self.regions[go_through_area[i]][1][1]
    ...
    def main(self):
        self.go_to_goal_area("kitchen", ["office", "bedroom"])
