        ...
        try:
            ...
            # If enough time has passed between recordings, consider the phrase complete. Clear audio buffer
                    if (now - self.time_last_input) > timedelta(seconds=self.phrase_timeout):
                            ...
                            try:
                                if self.save_data:
                                    ...
                                    # Save audio files and transcription to file
                                    # Should be saved in the Audio_files folder
                            ...
                            #py_trees.blackboard.Blackboard.set("spot_waiting_for_command", False) # RASMUS INDSÆT
                ...
                try: 
                    ...
                    wav_data = io.BytesIO(audio_data.get_wav_data())
                    # denoised_data = self.denoiser.filter(wav_data.read(),16000)
                    # denoised_data_2 = sr.AudioData(denoised_data,16000,2)
                    # wav_data_2 = io.BytesIO(denoised_data_2.get_wav_data())
                    # self.logger.info(type(wav_data_2))
                    # self.logger.info(type(wav_data))
                    #denoised_wav = self.denoiser.writ  
                    #########################       
                    with open(self.temp_file, 'w+b') as f:
                        f.write(wav_data.read())
                except Exception as e: 
